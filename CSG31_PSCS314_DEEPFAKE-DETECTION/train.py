    ...
    for batch_idx, (sequences, labels) in enumerate(dataloader):
        ...
        if batch_idx % 10 == 0:
            if torch.cuda.is_available():
                ...
                print(f'Batch {batch_idx}/{len(dataloader)}, Loss: {loss.item():.4f}, GPU Memory: {gpu_memory:.1f}MB')
            else:
                print(f'Batch {batch_idx}/{len(dataloader)}, Loss: {loss.item():.4f} (CPU)')
...
def main():
    ...
    # Automatic device detection - use GPU if available, otherwise CPU
