...
def visualize_sequence_gradcam(model, sequence, labels, original_frames=None, save_path='gradcam_visualization.png'):
    ...
    for i in range(5):
        if original_frames is not None and i < len(original_frames):
            # Use original video frame
            original_frame = original_frames[i]
            # Resize original frame to match heatmap
            original_resized = cv2.resize(original_frame, (224, 224))
            
            axes[0, i].imshow(original_resized)
            axes[0, i].set_title(f'Original Frame {i+1}')
            axes[0, i].axis('off')
            
            # Overlay heatmap on original frame
            overlayed = overlay_heatmap(original_resized, cam)
            axes[1, i].imshow(overlayed)
            axes[1, i].set_title(f'Original + Grad-CAM {i+1}')
            axes[1, i].axis('off')
        else:
            # Fallback to aligned frames
            ...
            axes[0, i].set_title(f'Aligned Frame {i+1}')
            axes[0, i].axis('off')
            
            # Grad-CAM overlay on aligned frame
            overlayed = overlay_heatmap(frame_uint8, cam)
            axes[1, i].imshow(overlayed)
            axes[1, i].set_title(f'Aligned + Grad-CAM {i+1}')
            axes[1, i].axis('off')
    
    title_suffix = "on Original Frames" if original_frames is not None else "on Aligned Faces"
    plt.suptitle(f'DeepFake Detection Explanation {title_suffix} (Label: {"FAKE" if labels.item() > 0.5 else "REAL"})', 
                 fontsize=16)
    plt.tight_layout()
...
def main():
    ...
    # Get original frames for overlay
    sample_path = dataset.samples[sample_idx]['path']
    original_frames = []
    
    # Extract original frames from video
    ...
    # Generate explanation with original frames
    if len(original_frames) >= 5:
        visualize_sequence_gradcam(model, sequence, label, original_frames, 'gradcam_original_frames.png')
        print('Grad-CAM on original frames saved as gradcam_original_frames.png')
    
    # Also generate standard aligned face version
    visualize_sequence_gradcam(model, sequence, label, None, 'gradcam_aligned_faces.png')
    print('Grad-CAM on aligned faces saved as gradcam_aligned_faces.png')
